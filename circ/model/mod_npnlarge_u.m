function [tnet,term,param,def,symb]=mod_npnlarge_u(net)
%
% p1 : beta
% p2 : Is
% p3 : n
...
%                     o  3
%
%    iD = Is/beta*(exp(vBE/(n*vT)) - 1)
%    iB = Is/beta*exp(vBE/(n*vT))
...
tnet=nan*ones(3,24);
tnet(1,1:6)=[11 1 4 Is/beta n vT];
tnet(2,1:6)=[ 6 4 3 2 3 beta];
tnet(3,1:4)=[ 2 1 4 Is/beta];

term={'B','C','E'};
param={'beta','Is','n','vT'};
c=1/2;
symb.patch{1}=c*(-[.3 .2 .2 .3]+j*[.75 .75 -.75 -.75]);  %base plate
symb.patch{2}=c*(([5/4 3/4 3/4]+j*[0 1/5 -1/5])*exp(j*atan2(-3/4,1))-(.25+.25j));    %the arrow
